<!-- TOC -->autoauto- [项目介绍](#项目介绍)auto            - [这个项目是我仿照公司的一个实际商业项目“国际机票订票系统”的前端部分，自己做的一个前端项目。](#这个项目是我仿照公司的一个实际商业项目国际机票订票系统的前端部分自己做的一个前端项目)auto            - [这个前端项目的主要内容包含三大部分部分：机票搜索（search），用户信息填写（fillinfo），订单生成界面（order）。](#这个前端项目的主要内容包含三大部分部分机票搜索search用户信息填写fillinfo订单生成界面order)auto    - [技术栈](#技术栈)auto    - [依赖模块](#依赖模块)auto    - [项目业务](#项目业务)auto        - [搜索界面](#搜索界面)auto        - [用户信息填写界面](#用户信息填写界面)auto        - [订单页面](#订单页面)auto    - [业务实现](#业务实现)auto    - [search页面](#search页面)auto        - [整个项目的页面布局结构，](#整个项目的页面布局结构)auto        - [航班信息的显示](#航班信息的显示)auto        - [搜索条件的实现](#搜索条件的实现)auto        - [search页面左边可伸缩筛选条件的实现。](#search页面左边可伸缩筛选条件的实现)auto        - [实现search页面跳转之后，回退search页面依然保留有用户之前输入的搜索条件。](#实现search页面跳转之后回退search页面依然保留有用户之前输入的搜索条件)auto        - [通过vue-router实现页面之间的跳转。](#通过vue-router实现页面之间的跳转)auto        - [在admin.vue中的aside上实现页面的全局跳转](#在adminvue中的aside上实现页面的全局跳转)auto    - [fillinfo页面](#fillinfo页面)auto        - [passenger_info_card组件](#passenger_info_card组件)auto        - [rule组件和baggage组件](#rule组件和baggage组件)auto        - [passenger_info_card组件](#passenger_info_card组件-1)auto        - [使用awsome图标库，配合element ui组件进行使用。美化界面](#使用awsome图标库配合element-ui组件进行使用美化界面)auto    - [oeder-list(订单列表页面)](#oeder-list订单列表页面)autoauto<!-- /TOC -->
...
axios拦截,配合路由钩子函数实现权限控制、登录、登出功能
